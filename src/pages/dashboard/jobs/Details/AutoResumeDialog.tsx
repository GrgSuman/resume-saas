...
interface AutoResumeDialogProps {
  ...
  resumeId: string;
  submitQuizToTailorResume: (resumeId: string, questionsWithAnswers: QuestionWithAnswer[]) => void;
}

const AutoResumeDialog = ({
  open,
  onOpenChange,
  jobTitle,
  jobId,
  questions,
  submitQuizToTailorResume,
  resumeId,
  ...
  // If questions are provided, skip to questions step
  useEffect(() => {
  ...
  const handleStartQuestions = async () => {
    ...
    try{
      ...
      const response = await axiosInstance.post(`/jobs/${jobId}/generate-resume-questions`, {
        resumeId: selectedResume,
      });
      setDemoQuestions(response.data.updatedJob?.resumeQuestions || []);
      setStep("questions");
    }
    catch(error){
  ...
  const handleQuizSubmit = async (questionsWithAnswers: QuestionWithAnswer[]) => {
    submitQuizToTailorResume(selectedResume, questionsWithAnswers);
    onOpenChange(false);
  };
          ...
          <div className="max-h-[90vh] min-h-[80vh] flex flex-col p-0 gap-0 overflow-hidden bg-white shadow-xl rounded-lg border border-slate-200">
            ...
            {step === "select" && (
              <DialogHeader className="px-6 pt-6 flex-shrink-0">
                ...
                <DialogDescription className="sr-only">
                  {`Select a resume to tailor for ${jobTitle}`}
                </DialogDescription>
              </DialogHeader>
            )}
            ...
            <div className={cn("flex flex-col flex-1 min-h-0 overflow-hidden", step === "select" && "my-4")}>
              {step === "select" ? (
                ...
                <Quiz
                  questions={questions.length > 0 ? questions : demoQuestions}
...
export default AutoResumeDialog;

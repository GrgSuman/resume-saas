...
const ModernTemplate = ({
  ...
  const SectionHeader = ({
    ...
    <div className="flex items-center justify-between mb-3">
      <div className="bg-gray-100  w-full px-3 py-1">
      ...
      {state.resumeEditingMode && (
        <Button
          ...
          className="h-6 px-2 mb-[8px] py-0.5 text-xs font-medium bg-blue-50 text-blue-700 border border-blue-300 hover:bg-blue-100"
        >
  ...
  const renderPersonalInfo = () => {
    ...
    return (
      <div className="mb-4">
        ...
        <div className="mt-4">
          <div className="bg-gray-100  w-full px-3 py-1 mb-2">
            <h2 className="font-bold uppercase">
              {
                settings.sections.find(
                  (section) => section.key === ResumeSectionKey.PERSONAL_INFO
                )?.label
              }
            </h2>
          </div>
          <p className="pl-3">{data.personalInfo.summary}</p>
        </div>
      </div>
    );
  };
  ...
  const renderExperience = () => {
    return (
      <div className="mb-4">
        <SectionHeader
          label={
            settings.sections.find((section) => section.key === "experience")
              ?.label
          }
  ...
  const renderEducation = () => {
    return (
      <div className="mb-4">
        <SectionHeader
          label={
            settings.sections.find((section) => section.key === "education")
              ?.label
          }
  ...
  const renderSkills = () => {
    return (
      <div className="mb-5">
        <SectionHeader
          label={
            settings.sections.find((section: { key: string; label: string; }) => section.key === "skills")?.label
          }
          sectionKey="skills"
        />
        <div className="space-y-2 pl-3">
          {data.skills?.map((skillGroup: { categoryName: string; items: { content: string }[]; }, index: number) => (
            <div key={index}>
  ...
  const renderProjects = () => {
    return (
      <div className="mb-4">
        <SectionHeader
          label={
            settings.sections.find((section) => section.key === "projects")
              ?.label
          }
          sectionKey="projects"
        />
          ...
          {data.projects?.map((project, index) => (
            <div key={index}>
              <div className="flex justify-between items-start mb-1">
                ...
                {project.link && (
                  <a href={project.link} className="block text-blue-500">
                    Project Link
                  </a>
                )}
              </div>
    ...
    return (
      <div className="mb-5">
        <SectionHeader
          label={
            settings.sections.find(
              (section) => section.key === "certifications"
            )?.label
          }
        ...
        <div className="pl-3">
          {data.certifications?.map((cert, index) => (
            <div key={index} className="mb-3">
              ...
              {cert.description && (
                <p className="mt-[2px]">{cert.description}</p>
              )}
            </div>
          ))}
        </div>
  ...
  const renderReferences = () => {
    return (
      <div className="mb-5">
        <SectionHeader
          label={
            settings.sections.find((section) => section.key === "references")
              ?.label
          }

...
const InterestSection = ({ interests  }: { interests: string[] }) => {
  ...
  return (
    <div className="mb-4">
      ...
      >
        <form className="space-y-6 font-sans" onSubmit={e => { e.preventDefault(); handleAddInterest(); }}>
          <div className="space-y-4 font-sans">
            <div>
              <label className="block text-sm font-medium text-gray-700 mb-1 font-sans">Interest</label>
              <input type="text" value={newInterest} onChange={e => setNewInterest(e.target.value)} onKeyDown={e => e.key === 'Enter' && handleAddInterest()} placeholder="e.g., Photography, Hiking, Tech Meetups" className="w-full border border-gray-300 rounded-lg px-3 py-2 focus:outline-none focus:ring-2 focus:ring-blue-400 focus:border-blue-400 bg-white shadow-sm transition font-sans" />
            </div>
          </div>
          <div className="pt-6 border-t border-gray-200 flex flex-col md:flex-row justify-end gap-3 bg-slate-50 -mx-6 px-6 pb-2 sticky bottom-0 z-10 font-sans">
            ...
            <button type="submit" className="px-4 py-2 bg-blue-600 text-white rounded-lg shadow hover:bg-blue-700 transition font-semibold w-full md:w-auto font-sans">Add Interest</button>
          </div>
        </form>
      </Modal>
    </div>
